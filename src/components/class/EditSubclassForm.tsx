import { Button, Checkbox, Col, Form, Input, InputNumber, Popover, Row, Select, Switch, Tag, Typography } from "antd";
import { PlusOutlined, DeleteOutlined, SaveOutlined, MinusOutlined, SearchOutlined, PictureOutlined } from '@ant-design/icons';

// utils
import { SpellPreparationTypeEnum } from "@/utils/enums";

// hooks
import useEditSubclass from "@/hooks/class/useEditSubclass";
...
// interfaces
import type { UpdateClassSubDTO, WorkshopClassSubDetailDataReturn } from "@/models/classInterfaces";
...
interface EditSubclassFormProps {
    subclassData: WorkshopClassSubDetailDataReturn | null;
    onCancel: () => void;
    onSubmit: (data: UpdateClassSubDTO) => Promise<void>;
    spells: WorkshopSpellReturn[];
}

export default function EditSubclassForm({ subclassData, onCancel, onSubmit, spells }: EditSubclassFormProps) {
    ...
    const {
        ...
        handleApplyPreset,
        presets,
    } = useEditSubclass(subclassData, onCancel, onSubmit, spells);

    const { t } = useTranslation();
    const spellPreparationType = Form.useWatch('spellPreparationType', editSubclassForm);

    if (!subclassData) {
        return <EditModalSkeleton />;
    }
    ...
    return (
        <Form
            name="editSubclass"
            layout="vertical"
            form={editSubclassForm}
            style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%', height: '100%' }}
            onFinish={submitEditSubclass}
        ...
        >
            <div style={{ display: 'flex', gap: '1.6rem', alignItems: 'flex-start' }}>
                <div style={styles.imageFormContainer}>
                    <ImageForm
                        ...
                        initialImage={subclassData.image || ""}
                    />
                </div>
                                                ...
                                                return (
                                                    <div key={index} style={styles.maxKnownCol}>
                                                        ...
                                                        <InputNumber
                                                            ...
                                                            onChange={(val: number | null) => handleMaxCantripKnown(index, val || 0)}
                                                            mode="spinner"
                                                            controls={false}
                                                        ...
                                                        {spellPreparationType !== SpellPreparationTypeEnum.PREPARED && (
                                                            <InputNumber
                                                                ...
                                                                onChange={(val: number | null) => handleMaxSpellKnown(index, val || 0)}
                                                                mode="spinner"
                                                                controls={false}
                                                            />
                                                        )}
            ...
            <div style={styles.formFooter}>
                ...
                <Button type="primary" htmlType="submit" loading={submitLoad} icon={<SaveOutlined />}>
                    {t('global.save')}
                </Button>
            </div>
        </Form>
