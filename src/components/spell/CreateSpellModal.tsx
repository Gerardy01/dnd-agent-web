...
// hooks
import useCreateSpell from "@/hooks/spell/useCreateSpell";
...
// interfaces
import type { CreateSpellDTO } from "@/models/spellInterfaces";
interface Props {
    open: boolean;
    onClose: () => void;
    onSubmit: (data: CreateSpellDTO) => Promise<void>;
}

const { Title, Text } = Typography;


export default function CreateSpellModal({ open, onClose, onSubmit }: Props) {
    ...
    const {
        ...
        submitCreateSpell,
        handleCloseModal,
    } = useCreateSpell(onClose, onSubmit);
    ...
    return (
        ...
        >
            <div style={styles.header}>
                <div>
                    <Title level={2} style={{ margin: '0px' }}>{t('spells.createSpellTitle')}</Title>
                    <Text style={{ fontSize: '1rem' }}>{t('spells.createSpellDescription')}</Text>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <Button
                        ...
                        type="primary"
                        onClick={() => createSpellForm.submit()}
                        loading={submitLoad}
                    >
                        {t('spells.createSpell')}
                    </Button>
            ...
            <div style={styles.content}>
                ...
                <div style={{ flex: '1' }}>
                    <Form
                        name="createSpell"
                        layout="vertical"
                        form={createSpellForm}
                        style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}
                        onFinish={submitCreateSpell}
