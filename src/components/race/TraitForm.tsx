...
// hooks
import useTraitForm from "@/hooks/race/useTraitForm";
...
export default function TraitForm({ initialValues, onSave, onCancel, featureTypeSelection, isEdit = false, isLoading = false, hasProgression = false }: Props) {
    const { t } = useTranslation();
    const { form, onFinish } = useTraitForm({ onSave });

    return (
        <div style={styles.bonusCard}>
            <div style={styles.bonusCardHeader}>
                <Text strong style={{ fontSize: '0.95rem' }}>
                    {isEdit ? t('races.editTrait') : t('races.addTrait')}
                </Text>
            ...
            <div style={styles.bonusCardContent}>
                ...
                >
                    <Form.Item
                        label={t('races.traitName')}
                        name="name"
                        rules={[{ required: true, message: t('global.fieldRequired') }]}
                        labelCol={{ style: { fontWeight: 'bold', fontSize: '0.85rem' } }}
                    >
                        <Input placeholder={t('races.traitNamePlaceholder')} maxLength={50} size="large" />
                    </Form.Item>
                    <Form.Item
                        label={t('races.traitDescription')}
                        name="description"
                        rules={[{ required: true, message: t('global.fieldRequired') }]}
                        labelCol={{ style: { fontWeight: 'bold', fontSize: '0.85rem' } }}
                    >
                        <Input.TextArea rows={4} placeholder={t('races.traitDescriptionPlaceholder')} maxLength={1250} showCount size="large" />
                    </Form.Item>
                    <Row gutter={16}>
                        {hasProgression && (
                            <Col span={12}>
